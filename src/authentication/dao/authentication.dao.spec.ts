...
describe('AuthenticationDao', () => {
	...
	describe('Expired', () => {
		it('should logout users with expired token', async () => {
			LoggedUserModel.deleteMany = jest.fn().mockResolvedValue([UserCollectionMock.logged[0]._id, UserCollectionMock.logged[1]._id]);
			await AuthenticationsDAO.expired([UserCollectionMock.logged[0]._id, UserCollectionMock.logged[1]._id]);
			expect(LoggedUserModel.deleteMany).toHaveBeenCalled();
		});
	});
});
